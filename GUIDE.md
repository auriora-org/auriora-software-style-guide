...
- **Local by default.** Device control and experiment capture SHOULD run with no Internet access, no cloud service and no vendor account, unless network connectivity is intrinsic to the workflow itself. Remote access, synchronization, collaboration and monitoring are welcome as options; a bench that stops working when a link drops, a subscription lapses or a provider disappears has acquired someone else's failure mode.
...
- **Walk the tree; take the Hub's word for its size.** Discovery starts at a root Hub's host-facing interface, reads the Hub's identity and capabilities — how many Module Ports and Hub-to-Hub link ports it has, their stable indices, per-port link state, the router's figures ([`AES-HUB-004`](https://github.com/auriora-org/auriora-engineering-standard/blob/main/docs/03-architecture.md#aes-hub-004-hub-identity-and-capability-discovery)) — and then visits every Module Port by path, finding a Module or a downstream Hub and recursing ([`AES-HUB-005`](https://github.com/auriora-org/auriora-engineering-standard/blob/main/docs/03-architecture.md#aes-hub-005-cascading-and-path-addressing)). The port count of a Hub comes from that Hub, never from a table of Hub products in the tool: a four-port and a sixteen-port Hub are configured by the same code path from the same readout.
- **Address by path, follow by identity.** The path of port indices from the root Hub is how a device is reached this session; it is stored as the topology attribute above, and it changes whenever a cable moves. The device is still the same device, keyed on its identity, and the tool re-resolves paths from identity after every rediscovery rather than reusing a path from yesterday.
- **Units come from the Module.** What a Module measures or drives is read from the Units it reports ([`AES-MCI-003`](https://github.com/auriora-org/auriora-engineering-standard/blob/main/docs/05-interfaces-and-versioning.md#aes-mci-003-module-capability-discovery)) — identity, execution model, capabilities, interface, and whether each was activated — never from the Module's product type. Show a refused Unit as refused with its reason; an interface that appears empty when hardware is plugged in is exactly what a user needs to see.
...
- **Verify arming; never trust a command.** Before an experiment that depends on several Modules acting together, the tool MUST confirm that every required Module actually reached the armed state, and MUST refuse to proceed — loudly, naming the Modules — when any has not. A control command MUST NOT be used as the simultaneous trigger for several Modules: the spread in arrival times is unbounded and unrecorded, so the data looks synchronized and is not. The deterministic event is an AEL event (§3.6).
- **Download by reading, not by session.** Stored objects — recordings, logs, committed Assets and Sessions — are retrieved as AES defines it ([AES-MCI-008](https://github.com/auriora-org/auriora-engineering-standard/blob/main/docs/05-interfaces-and-versioning.md#aes-mci-008-stored-object-retrieval)): enumerate the inventory, read the metadata, then read the object by identity, offset and length in chunks within the Module's declared maximum, checking that every response names the object, offset and length that were asked for. Keep the transfer state on this side: the byte count stored so far *is* the resume point, and after any interruption — cable, Hub, process — the tool rediscovers the Module by identity and continues from that offset without renegotiating anything. There is no Module-side "open" handle to model, because there is none.
- **Verified means matching the inventory hash.** A retrieved object is complete only when the assembled bytes match the integrity value the inventory reported; until then the local file is marked partial and never presented as a dataset. Store the verified hash with the file and in the deployment record. An object that vanished or changed mid-download is refused by the Module with a defined error: report it, keep the partial as partial, and do not retry against a different object under the same name.
- **Provenance comes from the Module, not from the connection.** A recording's run, segment, deployment, Session, time and sample range, format and firmware version are read from its metadata and stored with it. Whether it was made under this tool, under a Hub or alone in a greenhouse is not the tool's to infer and makes no difference to how it is read: one code path serves all three, and the recording of a run with two segments is two objects, never one file with a stitched seam.
- **Bulk never blocks the bench.** Retrieval is throughput work: run it behind control traffic at low priority, one outstanding chunk per link unless the binding allows more, and expect a control operation to any Module to be served within one chunk ([AES-HUB-003](https://github.com/auriora-org/auriora-engineering-standard/blob/main/docs/03-architecture.md#aes-hub-003-host-facing-interface-and-traffic-separation)). A user who arms a Module while a download runs on another port should not be able to tell.
- **Read-only, and say so.** Retrieval deletes nothing. Deleting a recording after download is a separate operation the Module must declare and the user asks for by name; the tool confirms it against the verified hash and refuses it while the run that produced the object is armed or running.
...
- **Record what ran.** An experiment record MUST contain the identity, firmware version and configuration of every Module that took part, plus the AEL topology, event-identifier map and committed route tables where AEL was used (§3.6). Reconstructing this afterwards from logs and memory is how irreproducible results happen.
...
### 3.6 Experiment Compilation and Deployment

These rules apply to software that turns an experiment into a running deployment — one Module on a local cable that will be left alone, or many Modules on Module Hubs — the host-side compiler, planner and orchestrator. The interfaces it targets are MCI, which prepares and observes state, and the AURIORA Event Link (AEL), which carries the deterministic typed events of the experiment ([Interfaces and Versioning §4](https://github.com/auriora-org/auriora-engineering-standard/blob/main/docs/05-interfaces-and-versioning.md#4-auriora-event-link)). AES fixes the interface invariants; everything below is how the host earns them. The Hub routes events and the Modules execute bound actions, but **the host owns the experiment's meaning**, and nothing in a Hub or a frame knows what an event is for. A single Module with no Hub and no AEL is deployed by the same steps — the route and identifier steps are simply empty; the validation, readback and record are not.

- **Compile, do not script.** Turn the high-level experiment description into a **deployment plan** — per-Module Sessions and Assets, a semantic-event → event-identifier map, per-Module AEL receive and source bindings, and per-Hub route tables — and deploy the plan. The researcher writes what should happen; the compiler produces the bindings and routes. A researcher MUST NOT have to author a route table or pick an identifier for a normal experiment. The description language is not standardized (the working name `AEDL` is informative); the plan is.
- **Bind to declared names.** The semantic events and actions a plan binds — `detector_a_fired`, `INSERT_MARKER` — are the logical event sources and receivable actions each Module declares over MCI ([`AES-MCI-003`](https://github.com/auriora-org/auriora-engineering-standard/blob/main/docs/05-interfaces-and-versioning.md#aes-mci-003-module-capability-discovery)), read at discovery. The compiler resolves the experiment's names against those declarations and refuses a plan that names a source or action a present Module does not declare; it never carries a per-product list of what a Module can emit.
- **Allocate identifiers per deployment.** Assign each logical event source one identifier, unique across the whole active AEL routing domain — every Module and every Hub the deployment interconnects, across Hub-to-Hub links — never `0x0000`, and never shared between two sources even when their events are semantically alike. Store the name ↔ identifier map in the deployment record before anything is armed; every logged `event_id` MUST be decodable from that record alone.
- **Scheduled actions stay local.** A known timeline — baseline, stimulus, pause, recovery at fixed offsets from `T0` — belongs in each Module's Session and runs on the Module's own clock. Route over AEL only what cannot be scheduled: the shared `T0`, detector-driven events, cross-Module reactions and completion markers whose actual time matters. A plan that expresses a fixed timeline as a stream of AEL events is wrong, not merely inefficient.
- **Answer one question before touching hardware: can this exact bench execute this exact experiment?** Validate the plan against what discovery reported and refuse deployment — naming the reason — when it cannot: every required Module present with the intended persistent identity; capabilities sufficient (AEL IN/OUT, AEL version, binding limits, Sessions, autonomous continuation where the experiment needs it); required Assets and Sessions available or transferable; every route realizable on the physical topology; Hub route-table, queue and event-rate capacity sufficient; the Hub-to-Hub topology acyclic; the accumulated latency along every event path within the experiment's tolerance given the devices' published envelopes; detector and action resources, storage and safety limits sufficient. A deployment that would work on a slightly different bench is a deployment that does not work.
- **Analyze the event graph for feedback.** Build the graph of source bindings → routes → receive bindings → actions → source bindings, and find its cycles. A cycle is a legitimate closed-loop design *only* when something bounds it — a detector refractory period, a maximum count, a state transition that disables the repeat, a timeout, hysteresis — and that guard lives in a Module's configuration, never in a Hub. Refuse a plan with an unguarded cycle; explain which one.
- **Deploy in order, verify each step.** Transfer what is missing (§3.5), stage every Hub's route table, validate, commit, and read back the committed content identities; then arm each Module and confirm each actually reached `ARMED`; only then may the experiment's initiating event occur — a host-released `T0` where the experiment has one, or the detector, external or Module event that starts it — and it is an AEL event, never a start command sent Module by Module. Any step that fails stops the sequence with the device and reason named; nothing is armed on a bench whose routes are not proven.
- **Unattended is a deployment, not a hope.** A Module that will run without the host is planned from what it declares ([AES-MCI-006](https://github.com/auriora-org/auriora-engineering-standard/blob/main/docs/05-interfaces-and-versioning.md#aes-mci-006-autonomous-continuation-and-deployment-policy), [AES-MCI-007](https://github.com/auriora-org/auriora-engineering-standard/blob/main/docs/05-interfaces-and-versioning.md#aes-mci-007-recovery-after-reset-and-run-segment-provenance)): verify that it declares autonomous continuation and every behavior the deployment needs — host loss, recovery after reset, storage exhaustion — and refuse by name when it does not; configure the deployment policy only from the declared behaviors and treat the Module's refusal of a combination as the answer, never as something to work around; where the Module exposes storage capacity and the Session's rate and duration are known, check that the deployment fits and say by how much; persist the chosen policy, exactly as read back, together with the Module's persistent-deployment-state identity in the deployment record; apply, read back, and arm only after the readback matches. The deployment policy is kept apart from the Session in the tool's own model too: the same Session deployed twice with different policies is one Session and two deployments. Nothing here is inferred from the product — an APEM that declares no recovery behavior gets none.
- **Show the autonomous behavior.** What the Module will do on host loss, after a power loss and when its storage fills is part of the active deployment and is displayed as such — in the UI, in the CLI's deployment summary and in the record — in the deployer's terms, never buried in defaults. A researcher who unplugs the cable knows, because the tool told them, whether the recording continues, whether a brownout starts a new segment, and whether the first day or the last day is lost when the disk fills.
- **Never touch routes during a run.** Once any dependent Module is `ARMED` or `RUNNING`, the tool MUST NOT stage-and-commit a replacement route table into the active slot; a new table is staged for later activation, and the only run-time route change is the defined abort or recovery path, which the tool exposes as such — loudly.
- **Route recovery is a deployment setting.** Whether a Hub restores its committed route table after a power loss is a policy the tool sets per Hub per deployment ([`AES-AEL-005`](https://github.com/auriora-org/auriora-engineering-standard/blob/main/docs/05-interfaces-and-versioning.md#aes-ael-005-active-hub-routing-and-bounded-overload-behavior)): *none* by default, *reinstate* only when the deployment is meant to run unattended with a Hub in the event path and the user has chosen so by name. Record the policy in the deployment record with the table's content identity; on reconnect, read whether a reinstatement happened and with which identity, and treat a Hub that reinstated a table the record does not know as a fault, not as a convenience.
- **Validate the topology you discovered, declare the rest.** The Hub tree and every cascaded link are known from discovery; an AEL-only link between two Hub link ports is not on it and is declared by the user before the plan is validated. Refuse a declared topology that would form a cycle, and refuse a plan whose event path depth exceeds what the summed per-hop envelopes allow.
- **Record what ran, completely.** The deployment record holds enough to reconstruct the run without anyone's memory: run identifier; every Module's persistent identity, hardware revision, firmware version, selected Session and Asset identities with their content hashes, committed bindings, and — where it declares autonomous continuation — its deployment policy as read back and its persistent-deployment-state identity; the AEL version; the event-identifier map; every Hub's identity, firmware version, topology and committed route-table identity; the devices' declared timing envelopes; and, after the run, every device's counters and event logs including ignored, invalid, unrouted and overflowed events, any abort, and every run segment with its boundary — reset cause, last and first local time and sample index, time-reference status, whether recovery was automatic and under which policy — together with storage-exhaustion events, any overwritten extent, and the identity and verified integrity value of every recording and log retrieved from the run. Counter reconciliation across sender, routers and receivers is how a lost event is located — build that comparison into the tool, not into a spreadsheet; segment reconciliation against the Module's own log is how a gap is located, and it belongs in the same place.
- **Reconnect by reading, not by assuming.** After the host loses and regains a device — a minute or a month later — rediscover by identity, read the actual lifecycle state, route-table identities and counters, and continue from what is true. Read as well whether a recovery occurred and how many, the current run and segment identity, the reset cause, whether the Module completed, stopped or refused a recovery and why, and whether storage was exhausted or data overwritten; then reconcile the record — new segments, an early completion, a refusal — against what was expected, and show the difference. Reconnection MUST NOT re-arm a Module, re-commit a route table or restart anything, and MUST NOT infer state from a USB path, a Hub port, the previous process's memory or the last thing the UI showed; a Module that declared autonomous continuation may have carried on or recovered under the policy the host itself set, and one that did not may have stopped — the record says which was expected, the device says what happened. A Module found waiting in `IDLE` with a recorded refusal is a question for the person, not a device to fix by re-arming.
- **Simulate the Hubs too.** The simulated bench (§3.4) includes AEL routers and links, not only Modules: frame delivery by route table, published latency and skew envelopes, queue capacity and overflow, corrupt and unrouted frames, Hub-to-Hub links and a host disconnect mid-run. That is what makes route-capacity refusal, feedback-loop refusal, lost-event location and no-re-arm-on-reconnect ordinary test cases.
...
Host software's threat model differs from firmware's: it handles developer machines, networks, credentials and update artifacts. Proportionality still rules — but the threat model MUST be written down, and "none" is a conclusion, not a default. A disposable script writes one line; software that updates firmware, accepts network connections, drives devices in a maintained workflow or stores research data writes the paragraph the controls below follow from.
...
*AURIORA Software Style Guide 0.3.0 — complements the AURIORA Engineering Standard. Licensed under CC BY-SA 4.0.*
